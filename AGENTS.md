...
1. **Регистрация:** Пользователь B кликает на реферальную ссылку: `t.me/bot?start=ref_{TELEGRAM_ID_A}`
2. Telegram передает `start_param=ref_{TELEGRAM_ID_A}` при первой загрузке
3. Frontend вызывает `/api/auth/init` с start_param
4. Backend устанавливает у User B значение `referred_by = TELEGRAM_ID_A` (неизменяемое)
